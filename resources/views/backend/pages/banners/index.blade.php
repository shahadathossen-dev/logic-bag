      ...
      <div class="col-md-10">
        <div class="card card-primary card-outline">
          <div class="card-header">
            <h3 class="card-title d-inline">Banners list</h3>
            @if (session('status'))
                ...
                @if(count($banners) > 0)
                  @foreach($banners as $item)
                    <tr>
                      ...
                      <td class="align-middle">
                        <img class="img-fluid" src="{{asset('/public/storage/backend/banners/thumbnail/'.$item->banner)}}" alt="{{$item->banner.' image'}}">
                      </td>
                      <td class="align-middle">{{ $item->created_at->format('H:i:s d-m-Y') }}</td>
                      <td class="align-middle">
                        <a class="btn btn-info" href="{{ route('admin.banner.edit', ['id' => $item->id]) }}" title="Edit Subcategory"><i class="far fa-edit"></i></a>
                        <a class="btn btn-success" href="{{ route('admin.banner.details', ['id' => $item->id]) }}" title="Subcategory details"><i class="far fa-check-square"></i></a>
                        <a class="btn btn-danger" href="{{ route('admin.banner.delete', ['id' => $item->id]) }}" title="Remove Subcategory"><i class="far fa-trash-alt"></i></a>
                      </td>
                    </tr>
                    @php
                      $sl++;
